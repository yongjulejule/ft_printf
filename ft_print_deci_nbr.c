...
/*   Updated: 2021/06/19 00:17:22 by yongjule         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	void	put_4bit_nbr(va_list cp_ap, t_lidx *strs)
{
	...
	{
		if (get_length_flag(strs) == 1)
			ft_putnbr_ushort_fd(va_arg(cp_ap, int), 1);
		else if (get_length_flag(strs) == 2)
			ft_putnbr_uchar_fd(va_arg(cp_ap, int), 1);
		else
			ft_putnbr_ui_fd(va_arg(cp_ap, int), 1);
	}
	va_end(cp_ap);
}
...
static	void	put_8bit_nbr(va_list cp_ap, t_lidx *strs)
{
	if (strs->txt[strs->opts.spec] == 'd' || strs->txt[strs->opts.spec] == 'i')
	{
		if (get_length_flag(strs) == 3)
			...
			ft_putnbr_ll_fd(va_arg(cp_ap, long), 1);
	}
	else
	{
		if (get_length_flag(strs) == 3)
			ft_putnbr_ul_fd(va_arg(cp_ap, long), 1);
		else if (get_length_flag(strs) == 4)
			ft_putnbr_ull_fd(va_arg(cp_ap, long), 1);
	}
	va_end(cp_ap);
}
